...
if __name__ == "__main__":
    ...
    if config is not None:
        ...
        logging.info('Connecting to ZABBIX')
        if not config['zabbix']['api'] and not config['zabbix']['username'] and not config['zabbix']['password']:
            logging.error('You should configure ZABBIX in config.json')
            sys.exit(1)
        r = requests.post(config["zabbix"]["api"] + "/api_jsonrpc.php",
                        json={
                            "jsonrpc": "2.0",
                            "method": "user.login",
                            "params": {
            ...
            if l['username'] not in [z['username'] for z in zabbix_admins]:
                r = requests.post(config["zabbix"]["api"] + "/api_jsonrpc.php",
                                        json={
                                            ...
                                            "params": {
                                                ...
                                                "name": l['name'],
                                                "surname": l['surname'],
                                                "passwd": "".join(random.choices(string.ascii_lowercase, k=20)),
                ...
                r = requests.post(config["zabbix"]["api"] + "/api_jsonrpc.php",
                        json={
                            ...
                            "method": "user.update",
                            "params": {
                                "userid": [z['uid'] for z in zabbix_admins if z['username'] == l['username']][0],
                                "username": l['username'],
                                "name": l['name'],
                                "surname": l['surname'],
                                "passwd": "".join(random.choices(string.ascii_lowercase, k=20)),
                ...
                if l['username'] not in [z['username'] for z in zabbix_users]:
                    ...
                    r = requests.post(config["zabbix"]["api"] + "/api_jsonrpc.php",
                            json={
                                ...
                                "method": "user.update",
                                "params": {
                                    "userid": [z['uid'] for z in zabbix_users if z['username'] == l['username']][0],
                                    "username": l['username'],
                                    "name": l['name'],
                                    "surname": l['surname'],
                                    "passwd": "".join(random.choices(string.ascii_lowercase, k=20)),
            ...
            if is_admin or is_user:
                r = requests.post(config["zabbix"]["api"] + "/api_jsonrpc.php",
                        json={
                            ...
                            "params": {
                                "userid": z['uid'],
                                "usrgrps": [{ "usrgrpid": admins_grpid if is_admin else users_grpid }],
                                "roleid": "1"
                            },
                            "auth": zab_token,
                            "id": 1
